...
void gui_cr_element(WINDOW *menu_win,WINDOW *menu_win_pre,int ptype){
    ...
    while(1){
        ...
        mvwprintw(menu_win, y-y+6, 2, "ENTER NEW VIN :");
        ...
        clrstdin();
        if(strlen(sz_name) == 0){
            mvwprintw(menu_win, 22, (x-strlen("CANCELED!"))/2 , "CANCELED!");
            wrefresh(menu_win);getch();clrstdin();
            continue;
        }
        mvwprintw(menu_win, 8, 2, "ENTER FULLNAME :");
        ...
        mvwprintw(menu_win, 10, 2, "ENTER MODEL :");
        ...
        mvwprintw(menu_win, 12, 2, "ENTER BRAND :");
        ...
        mvwprintw(menu_win, 15, (x-strlen("YOU ENTERED :"))/2, "YOU ENTERED :" );
        mvwprintw(menu_win, 17, 4, "%s ; %s",sz_name,sz_fullname);
        mvwprintw(menu_win, 18, 4, "%s ; %s",sz_model,sz_brand);
        mvwprintw(menu_win, 20, (x-strlen("IT'S OK? PRESS Y(YES)/N(NO) :"))/2, "IT'S OK? PRESS Y(YES)/N(NO) :");
        wrefresh(menu_win);
        ...
        if( (ynchar == 'Y') || (ynchar == 'y') || (ynchar == '\n' ) ) {
                ...
                mvwprintw(menu_win, 22, (x-strlen("CREATED!"))/2  , "CREATED!");
                wrefresh(menu_win);
...
void gui_cr_rscr(WINDOW *menu_win){
	...
	mvwprintw(menu_win, y - y + 2, (x-strlen("CREATE ELEMENT"))/2, "CREATE ELEMENT");
	mvwprintw(menu_win, 3, (x-strlen("WANA EXIT ? PRESS <-"))/2, "WANA EXIT ? PRESS <-");
