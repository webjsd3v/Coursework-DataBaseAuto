    ...
    while(1){
        ...
        switch(c_list){
            ...
            case 'e':
                ...
                gui_e_element(highlight_li,menu_wins[WIN_N_SETT],menu_wins[WIN_N_LIST],NULL); // включение панели редактировани€ элемента
                break;
            case 83 : // ^s  сохранить как
                guiconf_M_SAVE(menu_wins[WIN_N_SETT],menu_wins[WIN_N_LIST],WIN_N_SETT); // функци€ сохранитть как
                break;
            ...
            case 'l':
                ...
                break;
            case KEY_F(10): /* ѕрив€зки выхода их SAVE */
            case KEY_LEFT:
            case 27:        /* ESC KEY */
...
void print_list(WINDOW *menu_win, int highlight2,int n_choices_list,int enable_list,int ptype) // прорисовка списка
{
    ...
    switch(short_lang_data){
        case 1 :
            mvwprintw(menu_win, 15, 2, "---------- ѕодсказки ----------");
            mvwprintw(menu_win, 15 + 1, 2, "l,          # быстра€ загр. ");
            mvwprintw(menu_win, 15 + 2, 2, "l + SHIFT   # загрузка из ф. ");
            mvwprintw(menu_win, 15 + 3, 2, "<-,F10,ESC  # ¬ыход в глав. м.");
    ...
    wrefresh(menu_win);
    /* ѕроверка , выключено ли окно , если да , то вкл. */
